...
- [ ] **Testing E2E**: Ejecutar tests completos de flujos (registro→login→carrito→checkout→orden) con usuarios históricos (Camila 18 meses, Ignacio 17 días) y nuevos registros, verificar admin dashboard muestra timeline 2023-2024 correctamente
 - [x] ✅ **Revisar `/backend/src/utils/env.js`**: Validaciones básicas implementadas (`validateEnv`) — revisar y extender para producción si es necesario
 - [x] ✅ **Eliminar scripts de desarrollo antes de deploy**: `scripts/start-dev.sh` y `dev:all` removidos; revisar `/backend/scripts/` para otros helpers manuales
 - [x] ✅ **Actualizar cart/checkout/confirmation**: Checkout y createOrder integrados con `checkout.api` y flujo E2E probado (email + stock validation)
