...
async function seedCarts() {
  try {
    ...
    if (users.length) {
      ...
      await pool.query(
        `DELETE FROM carrito_items WHERE carrito_id IN (SELECT carrito_id FROM carritos WHERE usuario_id = ANY($1))`,
        [userIds],
      );
      await pool.query("DELETE FROM carritos WHERE usuario_id = ANY($1)", [userIds]);
    }
    ...
    for (const cart of CARTS) {
      ...
      const { rows } = await pool.query(
        `INSERT INTO carritos (usuario_id, status) VALUES ($1, $2) RETURNING carrito_id`,
        [userId, cart.status],
      );
      ...
      console.log(`Carrito creado para ${cart.email}`);
    }
