...
export default function OrdersAdminPage() {
  ...
  const hasActiveFilters = useMemo(() => {
    const { page: currentPage, limit, ...rest } = filters;
  ...
  const handleExport = useCallback(async (format) => {
    if (ordersData.length === 0) {
      notificationSystem.notify('No hay órdenes para exportar', 'info');
      return;
    }
    ...
    try {
      ...
      notificationSystem.notify(
        `${formatConfig?.label ?? format.toUpperCase()} listo para descargar`,
        'success'
      );
    } catch (error) {
      console.error('Error exporting orders:', error);
      notificationSystem.notify('Error al exportar pedidos', 'error');
    } finally {
      setIsExporting(false);
    }
  }, [filters, ordersData.length]);
