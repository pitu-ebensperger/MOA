  ...
  return (
    <nav aria-label="Paginación" className="flex items-center justify-center text-(--text-weak)">
      <button
        ...
        onClick={handlePrev}
        disabled={!canGoPrev}
        className="p-2 rounded-full text-inherit transition hover:bg-(--color-light-beige,#f6efe7) disabled:cursor-not-allowed disabled:opacity-40"
      ...
      <div className="mx-2 flex items-center gap-1 rounded-full border border-(--line,#e3ddd3) bg-transparent px-1 py-0.5 text-sm font-medium text-(--text-weak)">
        {pageItems.map((item) => {
          ...
          return (
            <button
              ...
              className={[
                ...
                isActive
                  ? "bg-(--color-light-beige,#f6efe7) text-(--color-primary-brown,#443114)"
                  : "hover:bg-(--color-light-beige,#f6efe7) hover:text-(--color-primary-brown,#443114)",
              ].join(" ")}
